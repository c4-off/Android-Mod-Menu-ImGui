...
bool InitBNMAssembly() {
...
#if UNITY_VER > 174
        if (assembly->aname.name == ("ByNameModding"))
            return true;
#else
    ...
    if (!BNM_assembly){
        ...
        BNM_Image->name = ("ByNameModding.dll");
#if UNITY_VER > 182
...
#endif
#endif
        BNM_Image->nameNoExt = ("ByNameModding");
#if UNITY_VER > 201
...
Il2CppClass* new_Class_FromName(const Il2CppImage* image, const char* namespaze, const char *name){
    ...
    if (!strcmp(image->name, ("ByNameModding.dll"))){
...
bool new_Class_Init(Il2CppClass *klass) {
    if (!klass)
        return false;
    if (strcmp(klass->image->name, ("ByNameModding.dll")))
...
void InitNewClasses(){
    for (int i = 0; i < Clases4Add->size(); i++){
        ...
        if (klass->Methods4Add != 0 && !klass->Methods4Add->empty()) {
            ...
            for (int m = 0; m < klass->Methods4Add->size(); m++) {
                ...
                for (int ip = 0; ip < method->thizMethod->parameters_count; ip++){
                    ...
                    newParam->name = (("arg") + to_string(ip)).c_str();
        ...
        *type;
        ...
        LOGIBNM(("[InitNewClasses] Added new class: [%s]::[%s] parent is [%s]::[%s]"), klass->GetNameSapce(), klass->GetName(), klass->GetBaseNameSapce(), klass->GetBaseName());
    }
    Clases4Add->clear();
}
