...
#include "../IL2CppResolver/Logger.h"
...
// For InputTextEx()
static bool             InputTextFilterCharacter(ImGuiInputTextState*state, unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min, bb.Max, col, true, 6);
...
bool ImGui::ButtonEx2(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags,ImColor colorr)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, colorr, true, 6);
...
bool ImGui::Button2(const char* label, const ImVec2& size_arg,ImColor colorr)
{
    return ButtonEx2(label, size_arg, ImGuiButtonFlags_None, colorr);
}
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    RenderArrow(window->DrawList, bb.Min + g.Style.FramePadding, text_col, window->Collapsed ? ImGuiDir_Down : ImGuiDir_Left, 1.0f);
...
/*
bool ImGui::Checkbox(const char* label, bool* v, void( *onCheckAct)(int, bool))
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::Checkbox(const char* label, bool* v, void( *onCheckAct)(int, bool))
{
    ...
    const ImRect check_bb(window->DC.CursorPos - ImVec2(5,5), window->DC.CursorPos + ImVec2(20,20));
    ...
    if (*v)
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(215, 219, 228), 3);
    else
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(36, 40, 49), 3);

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(255, 255, 255, 255));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Min.x + 30, check_bb.Min.y - 1), label);
    ImGui::PopStyleColor();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // Render shape
    const ImU32 frame_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    const ImU32 frame_col = GetColorU32(ImGuiCol_Border);
   // window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, frame_col, 20);
    // Drag behavior
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(160, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    if (!temp_input_is_active)
    {
        ...
        if (clicked || g.NavActivateId == id || g.NavActivateInputId == id)
        {
            ...
            FocusWindow(window);
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if (temp_input_allowed && ((clicked && g.IO.KeyCtrl) || g.NavActivateInputId == id))
            {
                temp_input_is_active = true;
    ...
    float grabv = (grab_bb.Max.x - grab_bb.Min.x)/2 + grab_bb.Min.x;
    float framev = frame_bb.Max.x - (grab_bb.Max.x - grab_bb.Min.x)/2;

    window->DrawList->AddLine(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 10), ImVec2(framev, frame_bb.Min.y + 10), ImColor(181 / 255.f, 183 / 255.f, 200 / 255.f), 3);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 10), ImVec2(grabv, frame_bb.Min.y + 10), ImColor(0, 100, 255), 3);


    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddCircleFilled(ImVec2(grabv, grab_bb.Min.y + 9), 5, ImColor(0, 100, 255), 360);
    window->DrawList->AddCircleFilled(ImVec2(grabv, grab_bb.Min.y + 9), 3, ImColor(33, 33, 33), 360);
    ...
    //ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(255, 255, 255, 255));
    // ImGui::PushFont(info);
    RenderTextClipped(ImVec2(grabv - 50, grab_bb.Min.y - 5 - 20 - 5), ImVec2(grabv + 50, grab_bb.Min.y - 5 + 20 - 5), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    //ImGui::PopFont();
    //ImGui::PopStyleColor();

    if (label_size.x > 0.0f)
        //ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(73 / 255.f, 80 / 255.f, 141 / 255.f, 1));
    RenderText(ImVec2(frame_bb.Max.x + 5, frame_bb.Min.y - 1), label);
   // ImGui::PopStyleColor();
...
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO,"ozMod_gui", __VA_ARGS__))
#define  LOGE(...)  __android_log_print(ANDROID_LOG_ERROR,"ozMod_gui",__VA_ARGS__)
...
static bool InputTextFilterCharacter(ImGuiInputTextState*state,unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    if(*p_char == 21){
        ...
        state->OnKeyPressed( STB_TEXTEDIT_K_LEFT);
        return false;
    }
    if(*p_char == 22){
    ...
    if(*p_char == 20){
        ...
        state->OnKeyPressed( STB_TEXTEDIT_K_DOWN);
        return false;
    }
    if(*p_char == 66){
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if ((flags & ImGuiInputTextFlags_AllowTabInput) && IsKeyPressedMap(ImGuiKey_Tab) && !ignore_char_inputs && !io.KeyShift && !is_readonly)
            if (!io.InputQueueCharacters.contains('\t'))
            {
                ...
                if (InputTextFilterCharacter(state,&c, flags, callback, callback_user_data, ImGuiInputSource_Keyboard))
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_validate_enter = IsKeyPressedMap(66);
        if(is_validate_enter){
           int *p = nullptr;
           *p = 2;
        }
        ...
        else if (IsKeyPressedMap(67) && !is_readonly)
        {
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                for (const char* s = clipboard; *s; )
                {
                    ...
                    if (c == 0)
                        break;
                    if (!InputTextFilterCharacter(state, &c, flags, callback, callback_user_data, ImGuiInputSource_Clipboard))
                        continue;
...
bool ImGui::TreeNode(const char* label)
{
    ...
    if (window->SkipItems)
        return false;
    return TreeNodeBehavior(window->GetID(label), ImGuiTreeNodeFlags_Framed, label, NULL);
}
